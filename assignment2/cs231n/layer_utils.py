...
def affine_relu_forward(x, w, b):
  ...
  cache = (fc_cache, relu_cache)  # 实际上，relu_cache == a
  return out, cache
...
def affine_bn_relu_dropout_forward(x, w, b, gamma, beta, bn_param, dropout_param):
  ...
  out, dropout_cache  = dropout_forward(rl, dropout_param)
...
def affine_bn_relu_dropout_backward(dout, cache):
  ...
  dp = dropout_backward(dout, dropout_cache)
  da = relu_backward(dp, relu_cache)
  dbn, dgamma, dbeta = batchnorm_backward_alt(da, bn_cache)
  dx, dw, db = affine_backward(dbn, fc_cache)
  return dx, dw, db, dgamma, dbeta
