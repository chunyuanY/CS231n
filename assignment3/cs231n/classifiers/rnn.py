...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    #backward(4)
    cache_idx -= 1
    dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dscores, caches[cache_idx])
    
    
    #backward(3)
    cache_idx -= 1
    if self.cell_type == "rnn":
        dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, caches[cache_idx])
    elif self.cell_type == "lstm":
        dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, caches[cache_idx])
    else:
        raise ValueError('%s not implemented' % (self.cell_type))
    
    #backward(2)
    cache_idx -= 1
    grads['W_embed'] = word_embedding_backward(dx, caches[cache_idx])
    
    #backward(1)
    cache_idx -= 1
    dfeatures, grads['W_proj'], grads['b_proj'] = affine_backward(dh0, caches[cache_idx])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    return loss, grads
  ...
  def sample(self, features, max_length=30):
    ...
    # a loop.                                                                 #
    ###########################################################################
    #foward(1) Embed the previous word using the learned word embeddings
    x0 = self._start * np.ones((N,1), dtype=np.int32)
    

    #forward(2) get the previous hidden state.
  ...
  def savemodel(self, path):
    ...
    for key, value in lstm_model.params.iteritems():
        file.create_dataset(key, data=value)
    file.close()

  def loadmodel(self, path, verbose=False):
